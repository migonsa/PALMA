...
class Test:
	def __init__(self, n_clients, n_servers=1, n_ports = 64, lambda_in=10, lambda_out=1./10, \
		...
		self.net = Mininet(topo=test_topo, host=CPULimitedHost, \
							switch=LinuxBridge)
		#self.net['s1'].sendCmd("sudo tshark -w results13.pcap -f 'ether proto 0x33ff'&");
		self.net.start()
		for h in range( 1, n_servers+1 ):
	...
	def run(self):
		for server in self.servers:
			self.popens[server] = server.popen(\
				"sudo ../server/palma-server -c ../configs/%s.xml -i %s-eth0" \
...
if __name__ == '__main__':
	setLogLevel( 'info' )
	Test(500, 0, test_time=300, output="results20.csv", lambda_in=20, lambda_out=0).run()
